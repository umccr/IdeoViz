...
#plotting
dev.off()
plotOnIdeo(chrom=seqlevels(testRanges), # which chrom to plot?
             ideoTable=ideo, # ideogram name
             values_GR=testRanges, # data goes here
             value_cols=1,
             val_range = c(0,2),
             col=brewer.pal(n=5, 'Spectral'), 
             plotType = 'rect', 
             ylab="array intensities",
...
#Here, I make an attempt to subset the default binned_multiSeries object to just chorosome X.Then I set the group 1 value to 1, and attempt to plot it. 

grX <- binned_multiSeries[seqnames(binned_multiSeries) == "chrX"]

grX$group1 = -1 
grX

plotOnIdeo(chrom=seqlevels(grX), # which chrom to plot?
           ideoTable=ideo, # ideogram name
           values_GR=grX, # data goes here
           value_cols=1, # col to plot
           col=brewer.pal(n=5, 'Spectral'), # colours
           # val_range=c(0,3),
           plotType = 'rect', # set y-axis range
